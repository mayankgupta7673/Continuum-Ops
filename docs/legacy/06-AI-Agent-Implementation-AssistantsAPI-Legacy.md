...
> This guide is kept for **historical reference only**. It documents the older **Assistants API** pattern (`Azure.AI.Projects`/`Azure.AI.OpenAI.Assistants` beta SDKs, manual thread creation, run polling, `RunStatus.RequiresAction` handling).
>
> **For the current, recommended implementation approach, see [../06-AI-Agent-Implementation.md](../06-AI-Agent-Implementation.md)**, which builds Diagnosis Agent and Verify Agent as **Microsoft Foundry Agent Service Prompt Agents** with tools exposed through a custom **MCP server**. That is the correct approach to build against today.
>
> This document remains relevant only if you later adopt a **Hosted Agent** (code-based agent, e.g. via Microsoft Agent Framework) and need a reference for manual thread/run/tool-call semantics. See [../08-AIOps-Solution-Architecture-Review.md](../08-AIOps-Solution-Architecture-Review.md) for the full Foundry Agents vs. custom-build rationale.
...
### Option 1: Microsoft Foundry Agent Service — Prompt Agents (Recommended for AI-Heavy Workloads)
- **Authoring**: Foundry portal (interactive) or SDK/REST (code-first, CI/CD-friendly)
- **Hosting**: Fully managed by Foundry — no compute, containers, or runtime code to maintain
- **Best for**: Diagnosis and Verify agents (AI reasoning + tool calling, no custom orchestration logic needed)
- **Benefits**: Managed tool-calling loop, session state, native tracing/evaluation, Agent Optimizer, automatic versioning/rollback

### Option 2: Custom Azure Functions Implementation (Recommended for Enterprise Integration)
- **Language**: .NET 8
- **Hosting**: Azure Functions Premium
- **Best for**: Repair agent and orchestration (deterministic business logic — never an LLM decision)
- **Benefits**: Full control, enterprise libraries, deterministic execution

### Hybrid Approach (Recommended for Continuum-Ops)
```mermaid
graph TB
    subgraph FoundryAgentService[Microsoft Foundry Agent Service - Prompt Agents]
        DIAG_AGENT[Diagnosis Agent<br/>Prompt Agent + MCP Tools]
        VERIFY_AGENT[Verify Agent<br/>Prompt Agent + MCP Tools]
    end
    
    subgraph AzureFunctions[Azure Functions - .NET 8]
        ORCHESTRATOR[Durable Functions Orchestrator]
        REPAIR_AGENT[Repair Agent]
        TOOLS[MCP Tool Server<br/>Enterprise Tool Registry]
